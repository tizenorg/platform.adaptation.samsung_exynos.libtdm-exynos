...
_tdm_exynos_display_get_mode(tdm_exynos_output_data *output_data)
{
    ...
    if (!output_data || !output_data->current_mode || !output_data->drm_modes)
    {
...
static tdm_error
_tdm_exynos_display_commit_primary_layer(tdm_exynos_layer_data *layer_data)
{
    ...
    if (output_data->mode_changed && layer_data->display_buffer_changed)
    {
        ...
        mode = _tdm_exynos_display_get_mode(output_data);
        if (!mode)
        {
...
static tdm_error
_tdm_exynos_display_commit_layer(tdm_exynos_layer_data *layer_data)
{
    tdm_exynos_data *exynos_data = layer_data->exynos_data;
    tdm_exynos_output_data *output_data = layer_data->output_data;
...
static void
_tdm_exynos_display_cb_vblank(int fd, unsigned int sequence,
...
exynos_display_handle_events(tdm_backend_data *bdata)
{
    ...
    ctx.vblank_handler = _tdm_exynos_display_cb_vblank;
    ctx.pp_handler = _tdm_exynos_display_cb_pp;
...
exynos_output_wait_vblank(tdm_output *output, int interval, int sync, void *user_data)
{
    ...
    ret = _tdm_exynos_display_get_cur_msc(exynos_data->drm_fd, output_data->pipe, &target_msc);
    if (ret != TDM_ERROR_NONE)
        goto failed_vblank;
    ...
    ret = _tdm_exynos_display_wait_vblank(exynos_data->drm_fd, output_data->pipe, &target_msc, vblank_data);
    if (ret != TDM_ERROR_NONE)
        goto failed_vblank;
...
exynos_output_commit(tdm_output *output, int sync, void *user_data)
{
    ...
    LIST_FOR_EACH_ENTRY(layer_data, &output_data->layer_list, link)
    {
        ...
        {
            ret = _tdm_exynos_display_commit_primary_layer(layer_data);
            if (ret != TDM_ERROR_NONE)
                return ret;
        }
        else
