...
_tdm_exynos_pp_set(tdm_exynos_pp_data *pp_data)
{
    ...
    memcpy(&property.config[0].pos, &info->src_config.pos, sizeof(tdm_size));
    ...
    memcpy(&property.config[1].pos, &info->dst_config.pos, sizeof(tdm_size));
...
static tdm_error
_tdm_exynos_pp_queue(tdm_exynos_pp_data *pp_data, tdm_exynos_pp_buffer *buffer, enum drm_exynos_ipp_buf_type type)
{
    tdm_exynos_data *exynos_data = pp_data->exynos_data;
